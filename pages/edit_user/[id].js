...
import s from '../../styles/EditUser.module.scss'
...
const EditUser = () => {
  ...
  const handleSubmit = () => {
    ...
    if (num % 2 !== 0) {
      ...
      patchData(`user/${editUser.id}`, { role }, auth.token).then((res) => {
        if (res.err)
  ...
  return (
    <div className="container">
      <div className={`${s.edit_user} my-3`}>
        <Head>
        ...
        <div>
          <button className="btn btn-dark" onClick={() => router.back()}>
            <i className="fas fa-long-arrow-alt-left" aria-hidden></i> Вернуться
          </button>
        </div>
